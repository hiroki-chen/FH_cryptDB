                        ...
                        const std::string &query)
{
     ...
     * debug
     */
    std::cout << "在函数dispatchOnLex()中\na.getDatabaseName(): " + a.getDatabaseName() + "\nquery: " + query << std::endl;
    /*
     * debug
     */
    try {
    ...
    if (noRewrite(*lex)) {
        ...
        try {
        	 ...
        	 * debug
        	 */
        	std::cout << "transformLex操作!" << std::endl;
                  ...
                  const std::string &default_db)
{
    ...
    std::cout << "in Rewriter::rewrite: " << q << std::endl;
    // output就是重写后的SQL语句集
...
Rewriter::decryptResults(const ResType &dbres, const ReturnMeta &rmeta)
{
    ...
    for (unsigned int c = 0; c < cols; c++) {
        ...
        for (unsigned int r = 0; r < rows; r++) {
            if (!fm || dbres.rows[r][c]->is_null() || false == fm->needEnc()) {
...
// 执行查询操作，包括SQL语句从写、执行SQL操作、获得结果、结果解密一整套过程
EpilogueResult
             ...
             SchemaCache *const schema_cache, bool pp)
{
    ...
    std::list<std::string> out_queryz; // 重写后的SQL语句
    ...
    for (auto it : out_queryz) {
        ...
        // qr->output->multipleResultSets() 判断是否有多个数据集结果
        /*
        TEST_Sync(ps.getConn()->execute(it, &dbres,
                                    qr->output->multipleResultSets()),
                  "failed to execute query!");
...
getItem(char *const content, enum_field_types type, uint len){
    ...
    if (IsMySQLTypeNumeric(type)) {
        ...
        std::cout << "in getItem IsMySQLTypeNumeric" << std::endl;
