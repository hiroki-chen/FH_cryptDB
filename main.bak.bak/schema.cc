...
FieldMeta::FieldMeta(const std::string &name, Create_field * const field,
    ...
    : fname(name), salt_name(BASE_SALT_NAME + getpRandomName()),
	  /* We pass the name of the field as the parameter to determine whether this field is
	   * sensitive or not.
	   */
      onion_layout(determineOnionLayout(m_key, field, sec_rating, name)),
...
{
	std::string iden = name.substr(0, 4);
	transform(iden.begin(), iden.end(), iden.begin(), ::toupper);
	this->need_enc = iden.compare(ENC_IDENTIFIER) == 0 ? true : false;
...
std::string FieldMeta::serialize(const DBObject &parent) const
{
    ...
    std::cout << "How fieldmeta is stored in the embedded database!" << serial << std::endl;
...
onionlayout FieldMeta::determineOnionLayout(const AES_KEY *const m_key,
											...
											std::string name)
{
    if (sec_rating == SECURITY_RATING::PLAIN || 0 != name.substr(0, 4).compare(ENC_IDENTIFIER)) {
        // assert(!m_key);
