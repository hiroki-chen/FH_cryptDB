...
connect(lua_State *const L)
{
    ...
    if (!ps)
    {
        ...
        const std::string mkey = "23090319013"; // XXX do not change as
...
rewrite(lua_State *const L)
{
    ...
    if (EXECUTE_QUERIES)
    {
        try
        {
            ...
            // If the array of UpdateOPe is NOT NULL,
            // we should call pro_insert into generate its corresponding
            // SQL query statement.
                       ...
                       int rows_index, ResType *const out_res)
{
    ...
    while (lua_next(L, rows_index))
    {
        ...
        while (lua_next(L, -2))
        {
            ...
            assert(key >= 0 && static_cast<uint>(key) < out_res->types.size());
...
int lua_cryptdb_init(lua_State *const L)
{
