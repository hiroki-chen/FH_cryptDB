 ...
 * ST_SERVER_VER_LEN:
 *
 * length of the server-version field in the binlog format description
 *
 ...
 * @NETWORK_MYSQLD_BINLOG_CHECKSUM_OFF: no checksums
 * @NETWORK_MYSQLD_BINLOG_CHECKSUM_CRC32: CRC32
 * @NETWORK_MYSQLD_BINLOG_CHECKSUM_UNDEF: undefined
 *
 * checksum used for the binlog
 */
typedef enum {
...
/**
 * network_mysqld_binlog_mode:
 * @BINLOG_MODE_UNSET: unset
 * @BINLOG_MODE_READ: read
 * @BINLOG_MODE_WRITE: write
 *
 * mode of the open binlog stream
 */
typedef enum {
	BINLOG_MODE_UNSET,
	BINLOG_MODE_READ,
 ...
 * @fd: file descriptor of a opened binlog
 * @log_pos: position in the binlog
 * @filename: name of the open binlog
 * @mode: file mode of the open binlog
 * @header_len: length of the binlog event header
 *
 * binlog context
 */
typedef struct {
	...
	network_mysqld_binlog_mode mode;
	gchar *filename;
