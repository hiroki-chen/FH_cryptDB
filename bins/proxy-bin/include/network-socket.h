 ...
 * @NETWORK_SOCKET_SUCCESS: success
 * @NETWORK_SOCKET_WAIT_FOR_EVENT: wait for a event to continue
 * @NETWORK_SOCKET_ERROR: error
 * @NETWORK_SOCKET_ERROR_RETRY: error, but try again
 *
 * return value for socket functions
 */
typedef enum {
 ...
 * network_socket:
 * @fd: socket-fd
 * @event: events for this fd
 * @src: getsockname()
 * @dst: getpeername()
 * @socket_type: SOCK_STREAM or SOCK_DGRAM for now
 * @last_packet_id: internal tracking of the packet_id's the automaticly set the next good packet-id
 * @packet_id_is_reset: internal tracking of the packet_id sequencing
 * @recv_queue: receive queue (each entry a valid mysql packet)
 * @recv_queue_raw: receive queue (each entry a packet as read from the network)
 * @send_queue: send queue
 * @challenge: handshake challenge
 * @response: handshake response
 * @is_authed: did a client already authed this connection
 * @default_db: default-db of the client
 *
 * socket with its queues and the mysql specific state-tracking
 */
typedef struct {
	int fd;
	struct event event;
...
NETWORK_API network_socket_retval_t network_socket_read(network_socket *sock);
...
NETWORK_API network_socket_retval_t network_socket_connect(network_socket *sock);
