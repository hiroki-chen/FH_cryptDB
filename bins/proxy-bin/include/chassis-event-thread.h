...
/**
 * chassis_event_op_type_t:
 * @CHASSIS_EVENT_OP_UNSET: unknown
 * @CHASSIS_EVENT_OP_ADD: add
 *
 * operation types for the #chassis_event_op_t
 */
typedef enum {
 ...
 * @chas: chassis
 * @notify_fd: wakeup fd
 * @notify_fd_event: event struct for the @@notify_fd
 * @thr: thread
...
CHASSIS_API void chassis_event_handle(int event_fd, gshort events, void *user_data);
CHASSIS_API void chassis_event_thread_set_event_base(chassis_event_thread_t *e, struct event_base *event_base);
CHASSIS_API void *chassis_event_thread_loop(chassis_event_thread_t *event_thread);

/**
 * chassis_event_threads_t:
 * @event_threads: (element-type chassis_event_thread_t): event threads
 * @event_queue: (element-type chassis_event_op_t): queue of event ops
 * @event_notify_fds: selectable pipe
 *
 * collection of #chassis_event_thread_t
 */
