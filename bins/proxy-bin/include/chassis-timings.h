 ...
 * @cycles: cycles
 * @ticks: ticks
 * @filename: file the timestamp was created from
 * @line: line number in the @file the timestamp was created in
 *
 * a timestamp with source-file-location
 */
typedef struct {
	const gchar *name;
	guint64 usec;
 ...
 * @timestamps: (element-type chassis_timestamp_t): timestamps
 *
 * collection of #chassis_timestamp_t
 */
typedef struct {
	GQueue *timestamps;
} chassis_timestamps_t;
