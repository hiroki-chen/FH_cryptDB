 ...
 * @users: (element-type GString GQueue): users
 * @max_idle_connections: max idling connections
 * @min_idle_connections: min idling connections
 *
 * a pool of idling, authenticated connections
 */
typedef struct {
	GHashTable *users;
 ...
 * @sock: the idling socket
 * @pool: a pointer back to the pool
 * @added_ts: at ... we want to make sure we don't hit wait_timeout
 *
 * entry in the #network_connection_pool
 */
typedef struct {
	network_socket *sock;
...
NETWORK_API GQueue *network_connection_pool_get_conns(network_connection_pool *pool, GString *username, GString *default_db);
