 ...
 * @udata: udata for @func
 *
 * a shutdown hook
 *
 * Deprecated: 0.9.0: use a GObject, store in a chassis' services hash and use ref-counting
 */
typedef struct {
...
CHASSIS_API void chassis_shutdown_hook_free(chassis_shutdown_hook_t *hook);

/**
 * chassis_shutdown_hooks_t:
 *
 * collection of #chassis_shutdown_hook_t
 */
typedef struct {
	/*< private >*/
	GMutex *mutex;
...
CHASSIS_API void chassis_shutdown_hooks_free(chassis_shutdown_hooks_t *hooks);
