 ...
 * @PROXY_NO_DECISION: no decission, use default behaviour
 * @PROXY_SEND_QUERY: send query as is
 * @PROXY_SEND_INJECTION: send injection
 * @PROXY_SEND_RESULT: send result
 * @PROXY_IGNORE_RESULT: ignore the result
 *
 * return values of hook functions
 */
typedef enum {
	PROXY_NO_DECISION,
	PROXY_SEND_QUERY,
 ...
 * @REGISTER_CALLBACK_SUCCESS: success
 * @REGISTER_CALLBACK_LOAD_FAILED: load failed
 * @REGISTER_CALLBACK_EXECUTE_FAILED: execute failed
 *
 * return value of the register callback functions
 */
typedef enum {
...
/**
 * network_mysqld_con_lua_injection:
 * @queries: An ordered list of queries we want to have executed.
 * @sent_resultset:  Flag to make sure we send only one result back to the client.
 *
