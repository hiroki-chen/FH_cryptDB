...
typedef struct ProxyState {
    ...
    std::vector<std::unique_ptr<UpdateOPE>> updateOPEs;
...
public:
    Analysis(const std::string &default_db, const SchemaInfo &schema)
        : pos(0), updateOPE(false), encoding_field_num(0), update(false),
		  special_update(false), db_name(default_db), schema(schema)  {}
    ...
    std::vector<std::unique_ptr<Create_field>> encoding_fields;

    bool updateOPE;
    unsigned int encoding_field_num;

    // Support for local_table which used to encrypt fh-OPE values.
    // Argument 1:dbname + tablename + columnanme; Argument2: STRING -> plaintext, UNSIGNED INT -> counter (0 by defualt)
    std::map<VariableLocator, std::map<double, unsigned int>, VLCmp> local_table;
    ...
    bool loadLocalTable(const rapidjson::Document &doc);
