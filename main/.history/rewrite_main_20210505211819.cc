...
RewriteOutput *
Rewriter::dispatchOnLex(Analysis &a, ProxyState &ps,
                        const std::string &query)
{
...
LEX *do_transform_where(const LEX &lex, Analysis &a)
{
    ...
    else
    {
        if (SQLCOM_DELETE == lex.sql_command || SQLCOM_UPDATE == lex.sql_command)
        {
            ...
            loadAllSaltsFromFile(a.getDatabaseName(), lex.query_tables, a);
...
// 进行重写操作
QueryRewrite
Rewriter::rewrite(ProxyState &ps, const std::string &q,
                  ...
                  const std::string &default_db)
{
    ...
    else
    {
        ...
        ps.updateOPEs = std::move(analysis.updateOPEs);
    }
