...
ProxyState::ProxyState(ConnectionInfo ci, const std::string &embed_dir,
    ...
    : updateOPE(false), masterKey(std::unique_ptr<AES_KEY>(getKey(master_key))),
                           ...
                           const std::unique_ptr<Connect> &e_conn)
{
    ...
    const EpilogueResult epi_result =
        executeQuery(*(const_cast<ProxyState *>(&this->ps)), select_q, this->default_db,
                     schema_cache.get());
                        ...
                        SchemaInfo const &schema) const
{
    ...
    const std::string re_delete =
        rewriteAndGetSingleQuery(*(const_cast<ProxyState *>(&ps)), delete_q, schema, this->default_db);
    ...
    const std::string re_insert =
        rewriteAndGetSingleQuery(*(const_cast<ProxyState *>(&ps)), insert_q, schema, this->default_db);
