...
Item *
getNewFunc(Item *const lhs, Item *const rhs, const Item_func::Functype &type, bool found)
{
    if (found)
    {
        switch (type)
...
rewrite_args_fh_ope(const Item_func *const item, Analysis &a)
{
    ...
    std::pair<unsigned int, bool> ret = findPos(a.local_table[VariableLocator(db_name, table_name, field_name)],
                                                *(item->arguments()[1]),
                                                item->functype());

    unsigned int pos = ret.first;
    ...
    return getNewFunc(anon_item_field, udf, item->functype(), found);
}
        ...
        const Item_func::Functype &type)
{
    ...
    bool edge = Item_func::Functype::LE_FUNC == type ||
                Item_func::Functype::GT_FUNC == type;

    int pos = -1;

    /**
     * Two cases:
     * 		> local_table does not contain such an element.
     * 		> local_table does contain such anm element.
     */

    auto iter = std::find_if(local_table.begin(), local_table.end(), [&](const std::pair<double, unsigned int> &item) {
        return item.first == val;
    });

    bool found = local_table.end() != iter;
    ...
    else
    {
        ...
        pos = edge ? pos + iter->second : pos + 1;
    }

    if (pos == -1)
    {
        // indicate that this cannot be satisfied.
        pos = UINT_MAX;
    }
    return std::make_pair((unsigned)pos, found);
}
