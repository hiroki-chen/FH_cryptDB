...
//create by zy   begin
my_bool   create_doc_init(UDF_INIT *const initid, UDF_ARGS *const args,
                                      char *const message);
char * create_doc(UDF_INIT *const initid, UDF_ARGS *const args,
                          char *const result, unsigned long *const length,
                          char *const is_null, char *const error);
void
create_doc_deinit(UDF_INIT *const initid);
                             ...
                             char *const message)
{
    if (args->arg_count != 0) 
    {
       strcpy(message, "error input number");
       return 1;
    }
      initid->maybe_null = 1;
      return 0;
                          ...
                          char *const is_null, char *const error)
{
     FILE *fp;
   // char *buf;
   //longlong res2 = *((longlong *)args->args[1]);
     if ((fp = fopen("qq","r+"))==NULL)
      {
        //strcpy(error, "can not open document");
      //buf[100] = "can not open document";
      //return static_cast<char*>(initid->ptr);
      memcpy(result,"nonono", 6);
      *length = 6;
      return result;
      }
...
void create_doc_deinit(UDF_INIT *const initid)
{
